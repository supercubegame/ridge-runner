"""Fresh one-use 12:23 diagnostic authorization; no lifecycle changes."""
...
b.BRANCH='audit/runtime-8061-v3-20260920'
b.CLAIM='claims/runtime-8061-v3-20260920-1223.json'
...
def claim():
 scope()
 data={'run_id':os.environ['GITHUB_RUN_ID'],'sha':os.environ['GITHUB_SHA'],'approved':'12:23 one fresh Basic 8061 endpoint, auto_start false, service max180s, local HTTP selfcheck, counters and timestamped public responses, delete only new endpoint; no Studio start/stop','at':b.now()}
 b.github('PUT','/contents/'+b.CLAIM,{'branch':'studio-results','message':'Claim approved 12:23 diagnostic trial','content':base64.b64encode(json.dumps(data).encode()).decode()})
 ...
 pathlib.Path('out').mkdir(exist_ok=True);pathlib.Path('out/runtime-v3-claim.json').write_text(json.dumps(data))
def test():
 b.test()
 d.test()
...
def execute():
 ...
 marker=json.dumps({'probe':'runtime-v3','nonce':secrets.token_hex(16)},sort_keys=True)
 r={'status':'FAIL','kind':'approved_runtime_v3_diagnostics','started_at_utc':b.now(),'checks':[],'cleanup':{},'service_lifetime_seconds':120,'public_attempts':[],'url_source':'user_API_Builder_screenshots_12:01','endpoint_url':URL,'studio_start_stop':'NOT_RUN','auto_wake':'NOT_RUN'}
 ...
 try:
  scope();receipt=json.loads(pathlib.Path('out/runtime-v3-claim.json').read_text())
  ...
  http.client.HTTPConnection.putrequest=original;r['finished_at_utc']=b.now();save();print('Runtime v3:',r['status'],'writes:',p.writes)

"""Fresh 12:47 authorization: omit only upstream instance_type."""
import base64,contextlib,copy,http.client,json,logging,os,pathlib,secrets,shlex,sys,time
import runtime_trial as b
import diagnostic_service as d
b.BRANCH='audit/runtime-8061-v4-20260920'
b.CLAIM='claims/runtime-8061-v4-20260920-1247.json'
...
def claim():
 scope()
 data={'run_id':os.environ['GITHUB_RUN_ID'],'sha':os.environ['GITHUB_SHA'],'approved':'12:47 one fresh Basic PLUGIN_API 8061 endpoint omit instance_type only, auto_start false, diagnostics max180s, delete only new endpoint; no machine switch or Studio start/stop','at':b.now()}
 b.github('PUT','/contents/'+b.CLAIM,{'branch':'studio-results','message':'Claim approved 12:47 single-variable trial','content':base64.b64encode(json.dumps(data).encode()).decode()})
 ...
 pathlib.Path('out').mkdir(exist_ok=True);pathlib.Path('out/runtime-v4-claim.json').write_text(json.dumps(data))
def test():
 b.test()
 d.test()
 import ast
 ...
 b.require(len(calls)==1 and {k.arg for k in calls[0].keywords}=={'cloudspace_id','port','auto_start','command','type'},'OMIT_INSTANCE_TYPE')
...
def execute():
 ...
 marker=json.dumps({'probe':'runtime-v4','nonce':secrets.token_hex(16)},sort_keys=True)
 r={'status':'FAIL','kind':'approved_runtime_v4_omit_instance_type','request_instance_type':'OMITTED','started_at_utc':b.now(),'checks':[],'cleanup':{},'service_lifetime_seconds':120,'public_attempts':[],'url_source':'user_API_Builder_screenshots_12:01','endpoint_url':URL,'studio_start_stop':'NOT_RUN','auto_wake':'NOT_RUN'}
 ...
 def inspect_service():
  ...
  while time.monotonic()<deadline:
   ...
   if result.exit_code is not None and result.exit_code!=-1:
    ...
    b.require(set(data)<= {'status','bound','lifetime_seconds','before','after','elapsed_seconds','error_type','diagnostics'},'REMOTE_OUTPUT')
 ...
 try:
  scope();receipt=json.loads(pathlib.Path('out/runtime-v4-claim.json').read_text())
  ...
  with open(os.devnull,'w') as sink,contextlib.redirect_stdout(sink),contextlib.redirect_stderr(sink):
   ...
   try:
    ...
    body=EndpointServiceCreateEndpointBody(name=b.NAME,ports=['8061'],auth=V1EndpointAuth(enabled=True,username=username,password=password,user_api_key=False),cloudspace=V1UpstreamCloudSpace(cloudspace_id=b.SID,port='8061',auto_start=False,command='',type=V1EndpointType.PLUGIN_API))
    wire=client.api_client.sanitize_for_serialization(body)
    b.require('instanceType' not in wire['cloudspace'] and 'instance_type' not in wire['cloudspace'],'WIRE_INSTANCE_TYPE_NOT_OMITTED')
    ...
    b.require(live.cloudspace.auto_start is False and not live.cloudspace.command and live.cloudspace.type==V1EndpointType.PLUGIN_API,'UPSTREAM_CONFIG')
    c=live.cloudspace
    ...
    command='exec env -i PATH=/usr/local/bin:/usr/bin:/bin python3 -c '+shlex.quote(d.remote_source(marker=marker))
    p.armed='command'
    ...
    stage='diagnostic_http';deadline=time.monotonic()+70;ready=False;denied=False
    def observe(label,header,path='/probe'):
     entry={'label':label,'at_utc':b.now()};start=time.monotonic()
     try:
      code,data=b.request(URL+path,header)
     ...
     return entry
    missing=observe('missing',None)
    wrong=observe('wrong','Basic '+base64.b64encode(b'wrong:wrong').decode())
    while time.monotonic()<deadline:
     entry=observe('valid',auth)
     if entry.get('http_status')==200 and entry.get('exact_marker'):ready=True;break
     if entry.get('http_status') in (401,403):break
     time.sleep(3)
    missing=observe('missing_final',None)
    wrong=observe('wrong_final','Basic '+base64.b64encode(b'wrong:wrong').decode())
    denied=all(e.get('http_status') in (401,403) and e.get('exact_marker') is False for e in (missing,wrong))
    r['authenticated_marker_verified']=ready;r['missing_wrong_denied_observed']=denied
    if ready:r['checks'].append('valid_basic_auth_exact_marker')
    if denied:r['checks'].append('missing_and_wrong_auth_denied_http_status')
    root=observe('root_valid',auth,'/')
    r['trial_status']='PASS' if ready and denied and root.get('http_status')==404 else 'FAIL'
    stage='wait_service_exit';inspect_service()
    b.require(r.get('service',{}).get('exit_code')==0 and r['service'].get('status')=='EXPIRED_CLOSED' and r['cleanup'].get('8061_no_listeners_after') and r['service'].get('diagnostics',{}).get('local_http_ok'),'SERVICE_EXIT_OR_LOCAL_HTTP')
  ...
  http.client.HTTPConnection.putrequest=original;r['finished_at_utc']=b.now();save();print('Runtime v4:',r['status'],'writes:',p.writes)

"""Fresh 16:54 approval: minimal PLUGIN_PORT baseline, not a causal experiment."""
...
b.BRANCH='audit/runtime-8061-v6-20260920'
b.CLAIM='claims/runtime-8061-v6-20260920-1654.json'
...
def claim():
 scope()
 data={'run_id':os.environ['GITHUB_RUN_ID'],'sha':os.environ['GITHUB_SHA'],'approved':'16:54 one fresh Basic PLUGIN_PORT endpoint; omit command, user_api_key, instance_type; auto_start false; TCP/HTTP diagnostics max180s; delete only new endpoint; no original changes, machine switch or Studio start/stop; abort on conflict/not Running/unsupported auth; not single-variable causation','at':b.now()}
 b.github('PUT','/contents/'+b.CLAIM,{'branch':'studio-results','message':'Claim approved 16:54 minimal diagnostic trial','content':base64.b64encode(json.dumps(data).encode()).decode()})
 ...
 pathlib.Path('out').mkdir(exist_ok=True);pathlib.Path('out/runtime-v6-claim.json').write_text(json.dumps(data))
def test():
 b.test()
 d.test()
 import ast
 ...
 b.require(len(calls)==1 and {k.arg for k in calls[0].keywords}=={'enabled','username','password'},'MINIMAL_AUTH')
...
def execute():
 ...
 marker=json.dumps({'probe':'runtime-v6','nonce':secrets.token_hex(16)},sort_keys=True)
 r={'status':'FAIL','kind':'approved_runtime_v6_minimal_instrumented','omitted_request_fields':['command','userApiKey','instanceType'],'causation':'NOT_A_SINGLE_VARIABLE_TEST','started_at_utc':b.now(),'checks':[],'cleanup':{},'service_lifetime_seconds':120,'service_hard_kill_seconds':165,'public_attempts':[],'url_source':'user_API_Builder_screenshots_12:01','endpoint_url':URL,'studio_start_stop':'NOT_RUN','auto_wake':'NOT_RUN'}
 ...
 try:
  scope();receipt=json.loads(pathlib.Path('out/runtime-v6-claim.json').read_text())
  ...
  with open(os.devnull,'w') as sink,contextlib.redirect_stdout(sink),contextlib.redirect_stderr(sink):
   ...
   try:
    ...
    body=EndpointServiceCreateEndpointBody(name=b.NAME,ports=['8061'],auth=V1EndpointAuth(enabled=True,username=username,password=password),cloudspace=V1UpstreamCloudSpace(cloudspace_id=b.SID,port='8061',auto_start=False,type=V1EndpointType.PLUGIN_PORT))
    wire=client.api_client.sanitize_for_serialization(body)
    b.require(set(wire['cloudspace'])=={'cloudspaceId','port','autoStart','type'} and set(wire['auth'])=={'enabled','username','password'},'WIRE_MINIMAL_KEYS')
    b.require(wire['cloudspace']['autoStart'] is False and wire['cloudspace']['type']=='ENDPOINT_PLUGIN_PORT' and wire['auth']['enabled'] is True,'WIRE_VALUES')
    r['wire_keys']={'cloudspace':sorted(wire['cloudspace']),'auth':sorted(wire['auth'])}
  ...
  http.client.HTTPConnection.putrequest=original;r['finished_at_utc']=b.now();save();print('Runtime v6:',r['status'],'writes:',p.writes)

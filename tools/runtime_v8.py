"""Approved 19:18 v8: current Studio cluster_id only; exact cleanup."""
...
b.BRANCH='audit/runtime-8061-v8-20260920'
b.CLAIM='claims/runtime-8061-v8-20260920-1918.json'
APPROVAL='19:18 approve_v8_cloud_trial_1918; one temporary8061 PLUGIN_PORT plus fresh Studio clusterId; omit auth; autoStart false; one anonymous diagnostic command120s/hard165s; exact new endpoint deletion; abort not Running/missing cluster/conflict/config drift/unexpected auth; preserve8060/machine/sleep/SSH; no lifecycle or replay'
URL='https://8061-01m2watakghqmc1fnymkg5czax.cloudspaces.litng.ai'
def cluster_for_request(studio):
 value=getattr(studio,'cluster_id',None)
 b.require(isinstance(value,str) and bool(value) and value==value.strip() and not any(c.isspace() for c in value),'MISSING_OR_INVALID_STUDIO_CLUSTER')
 return value
...
def scope():
 b.require(APPROVAL is not None,'NO_FRESH_AUTHORIZATION')
...
def claim():
 scope()
 data={'run_id':os.environ['GITHUB_RUN_ID'],'sha':os.environ['GITHUB_SHA'],'approved':APPROVAL,'at':b.now()}
 b.github('PUT','/contents/'+b.CLAIM,{'branch':'studio-results','message':'Claim approved cluster-only comparison','content':base64.b64encode(json.dumps(data).encode()).decode()})
 ...
 pathlib.Path('out').mkdir(exist_ok=True);pathlib.Path('out/runtime-v8-claim.json').write_text(json.dumps(data))
def test():
 b.test()
 d.test()
 import ast
 ...
 b.require(len(calls)==1 and {k.arg for k in calls[0].keywords}=={'cloudspace_id','port','auto_start','type','cluster_id'},'CLUSTER_ONLY_UPSTREAM_DELTA')
 ...
 for value in (None,'',' ','fixture cluster',' fixture-cluster','fixture-cluster\n',123):
  try:cluster_for_request(N(cluster_id=value))
  except RuntimeError:pass
  else:raise AssertionError('INVALID_CLUSTER_ACCEPTED')
 b.require(cluster_for_request(N(cluster_id='fixture-cluster'))=='fixture-cluster','EXACT_CLUSTER')
...
def execute():
 ...
 marker=json.dumps({'probe':'runtime-v8','nonce':secrets.token_hex(16)},sort_keys=True)
 r={'status':'FAIL','kind':'approved_runtime_v8_cluster_only','omitted_request_fields':['auth','command','instanceType','terminalSessionId','customDomain'],'causation':'CLUSTER_ONLY_REQUEST_DELTA_NOT_CAUSAL_PROOF','started_at_utc':b.now(),'checks':[],'cleanup':{},'service_lifetime_seconds':120,'service_hard_kill_seconds':165,'public_attempts':[],'url_source':'user_API_Builder_screenshots_12:01','endpoint_url':URL,'studio_start_stop':'NOT_RUN','auto_wake':'NOT_RUN','ssh':'NOT_USED'}
 ...
 try:
  scope();receipt=json.loads(pathlib.Path('out/runtime-v8-claim.json').read_text())
  b.require(receipt['sha']==os.environ['GITHUB_SHA'] and receipt['run_id']==os.environ['GITHUB_RUN_ID'],'CLAIM')
  b.require(receipt['approved']==APPROVAL,'APPROVAL_MATCH')
  ...
  with open(os.devnull,'w') as sink,contextlib.redirect_stdout(sink),contextlib.redirect_stderr(sink):
   ...
   try:
    ...
    current_cluster=cluster_for_request(s._studio)
    private.append(current_cluster)
    r['cluster_input']={'source':'fresh_Studio_response','present':True,'value':'REDACTED'}
    ...
    body=EndpointServiceCreateEndpointBody(name=b.NAME,ports=['8061'],cloudspace=V1UpstreamCloudSpace(cloudspace_id=b.SID,port='8061',auto_start=False,type=V1EndpointType.PLUGIN_PORT,cluster_id=current_cluster))
    wire=client.api_client.sanitize_for_serialization(body)
    b.require(set(wire)=={'name','ports','cloudspace'} and set(wire['cloudspace'])=={'cloudspaceId','port','autoStart','type','clusterId'},'WIRE_CLUSTER_ONLY_KEYS')
    b.require(wire['cloudspace']['clusterId']==current_cluster,'WIRE_EXACT_STUDIO_CLUSTER')
    ...
    c=live.cloudspace
    b.require(not c.cluster_id or c.cluster_id==current_cluster,'READBACK_OTHER_CLUSTER')
  ...
  http.client.HTTPConnection.putrequest=original;r['finished_at_utc']=b.now();save();print('Runtime v8:',r['status'],'writes:',p.writes)

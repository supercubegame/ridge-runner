"""Inspect endpoint schemas without platform credentials or requests."""
import ast,contextlib,inspect,json,logging,os,pathlib,socket,sys,textwrap
r={'status':'FAIL','kind':'isolated_endpoint_design_schema','network_attempts':0,'platform_requests':0,'methods':{},'models':{}}
def deny(*a,**k):
...
try:
 ...
 with open(os.devnull,'w') as sink,contextlib.redirect_stdout(sink),contextlib.redirect_stderr(sink):
  ...
  names=['endpoint_service_create_endpoint','endpoint_service_update_endpoint','endpoint_service_delete_endpoint','endpoint_service_get_endpoint','cloud_space_service_get_cloud_space_instance_open_ports']
  model_names={'EndpointServiceCreateEndpointBody','EndpointServiceUpdateEndpointBody','V1UpstreamCloudSpace','V1Endpoint'}
  for name in names:
   cls=api.CloudSpaceServiceApi if name.startswith('cloud_space') else api.EndpointServiceApi
   fn=getattr(cls,name+'_with_http_info');src=textwrap.dedent(inspect.getsource(fn));tree=ast.parse(src)
   calls=[n for n in ast.walk(tree) if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and n.func.attr=='call_api'];assert len(calls)==1
   c=calls[0];path,verb=[ast.literal_eval(n) for n in c.args[:2]]
   response=next((ast.literal_eval(k.value) for k in c.keywords if k.arg=='response_type'),None)
   params=[]
   ...
   if response:model_names.add(response)
  for depth in range(4):
   ...
   for name in sorted(model_names):
    ...
    if not fields:r.setdefault('enum_sources',{})[name]=inspect.getsource(obj)
    for typ in fields.values():
     typ=typ.replace('list[','').replace(']','')
     if typ.startswith(('V1','Externalv1','EndpointService')):next_names.add(typ)
